...
export const Board = ({ boardState, handleCellClick, placeFlag, board, gameOver, gameWon }) => {
    let ind = 0;
    let arrayBoard = []
    if (board.length * board[0].length === boardState.nLines * boardState.nColumns) {
        for (let i = 0; i < boardState.nLines; i++) {
            ...
            for (let j = 0; j < boardState.nColumns; j++) {
                row.push(< Cell key={ind++} x={i} y={j} handleCellClick={handleCellClick} placeFlag={placeFlag} cellState={board[i][j]} gameOver={gameOver} gameWon={gameWon} />)
            }
